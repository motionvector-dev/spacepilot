...
export function WebSshTerminalView({ 
  ...
  const containerClasses = isFullscreen
    ? 'fixed inset-0 z-50 bg-ground p-6 flex flex-col'
    : 'bg-surface border border-line-200 rounded-2xl overflow-hidden flex flex-col';

  return (
    <div className={containerClasses}>
      {/* Terminal Top Window Bar */}
      <div className="h-11 bg-raised border-b border-line-200 px-4 flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center bg-inset border border-line-200 rounded-lg p-0.5 ml-2">
            <button
            ...
            >
              <Radio className="w-3 h-3 text-ink-500" />
            ...
            >
              <TerminalIcon className="w-3 h-3 text-ink-500" />
        ...
        <div className="flex items-center gap-2">
          {/* Status Indicator */}
          <span className="flex items-center gap-1 text-[10.5px] font-mono text-ink-500 mr-1 hidden sm:flex">
            {isConnected ? (
          ...
          <div className="hidden md:flex items-center gap-1 border-r border-line-200 pr-2 mr-1">
            <button
              onClick={() => handleServiceAction('restart_worker')}
              disabled={inspectActionMutation.isPending}
              className="text-[10.5px] font-mono font-semibold px-2 py-0.5 rounded bg-inset text-ink-700 hover:text-ink hover:bg-strong transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            ...
            <button
              ...
              disabled={inspectActionMutation.isPending}
              className="text-[10.5px] font-mono font-semibold px-2 py-0.5 rounded bg-inset text-danger/80 hover:text-danger hover:bg-danger-soft transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
          ...
          {/* Font-Size Scaling */}
          <div className="flex items-center bg-inset border border-line-200 rounded-md p-0.5">
            <button
              onClick={decreaseFontSize}
              className="p-1 rounded text-ink-500 hover:text-ink transition-colors cursor-pointer"
              title="Decrease Font Size"
            >
              <ZoomOut className="w-3.5 h-3.5" />
            </button>
            <span className="text-[10px] font-mono px-1.5 text-ink-700 font-bold select-none">
              {fontSize}px
            </span>
            <button
          ...
          <button
            ...
            className={`p-1.5 rounded border transition-all cursor-pointer ${
              isPaused
                ...
                : 'border-line-200 text-ink-500 hover:text-ink hover:bg-strong'
            }`}
            title={isPaused ? 'Resume Stream (paused — not receiving live output)' : 'Pause Stream'}
          >
          ...
          <button
            onClick={clearBuffer}
            className="p-1.5 rounded border border-line-200 text-ink-500 hover:text-ink hover:bg-strong transition-all cursor-pointer"
      ...
      >
        {lines.length === 0 ? (
          <div className="text-ink-300 italic py-8 text-center flex items-center justify-center gap-2">
          ...
          lines.map((line, idx) => (
            <div
              ...
              className={`whitespace-pre-wrap ${
                ...
                line.includes('[CONNECTED]') || line.includes('success') ? 'text-verify' :
                line.includes('[Cockpit]') ? 'text-ink-900' :
                line.includes('warning') ? 'text-ink' :
                'text-ink-700'
              }`}
            >
              {line}
        ...
        {activeTab === 'shell' && (
          <form onSubmit={handleCommand} className="flex items-center gap-1.5 text-ink mt-2">
            <span className="text-ink-900 font-bold select-none">ubuntu@gpu-box:~$</span>
            <input
              type="text"
              value={inputVal}
              onChange={(e) => setInputVal(e.target.value)}
              placeholder="Enter shell command (e.g. nvidia-smi, df -h, tail -f /scratch/worker/worker.log)"
              className="flex-1 bg-transparent border-none outline-none text-ink font-mono focus:ring-0 p-0"
              autoFocus
            />
          </form>
        )}
