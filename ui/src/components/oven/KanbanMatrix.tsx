...
export function KanbanMatrix() {
  return (
    <main className="flex-1 grid grid-cols-5 gap-4 p-4 lg:p-5 overflow-x-auto overflow-y-hidden bg-surface min-w-[1200px]">
      {/* Column 1: Backburner */}
      <section className="bg-raised border border-line-200 rounded-xl flex flex-col h-full overflow-hidden">
        <div className="p-3.5 px-4 border-b border-line-200 flex items-center justify-between shrink-0">
          <div className="font-mono text-xs font-extrabold uppercase tracking-wider text-ink-500 flex items-center gap-1.5">
          ...
          <span className="font-mono text-[11px] font-bold bg-strong px-2 py-0.5 rounded-full text-ink-700">3</span>
        </div>
      ...
      <section className="bg-raised border border-line-200 rounded-xl flex flex-col h-full overflow-hidden">
        <div className="p-3.5 px-4 border-b border-line-200 flex items-center justify-between shrink-0">
          ...
          <span className="font-mono text-[11px] font-bold bg-strong px-2 py-0.5 rounded-full text-ink-700">2</span>
        </div>
        <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-3">
          <KanbanCard priority="p1" badgeText="P1 · Wave 5" title="1-Click LoRA Studio" description="Upload 10-20 reference images in Cockpit, run spot PEFT training ($0.40), hot-swap adapter." persona="LoRA Agent" status="In Progress" statusColor="var(--text-900)" borderColor="var(--border-400)" />
          <KanbanCard priority="p1" badgeText="P1 · Wave 5" title="Spot Training & R2 Checkpoints" description="Multi-node training recipes with preemption recovery and loss curve telemetry streaming." persona="MLOps" status="In Progress" statusColor="var(--text-900)" borderColor="var(--border-400)" />
        </div>
      </section>
      ...
      <section className="bg-raised border border-line-200 rounded-xl flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-3">
          <KanbanCard priority="p0" badgeText="P0 · Review Gate" title="Zero-Leniency Audit SLA" description="Rigorous review of WebSocket PTY bridge, thread safety locks, and path traversal confinement." persona="Opus 4.6" status="Passed (100/100)" statusColor="var(--text-900)" borderColor="var(--border-400)" />
          <KanbanCard priority="p1" badgeText="P1 · Review Gate" title="Polar.sh + x402 Header" description="Autonomous agent USDC micropayment verification and MoR webhook reconciliation." persona="Settlement" status="Pending" statusColor="var(--text-900)" borderColor="var(--border-400)" />
        </div>
      </section>
      ...
      <section className="bg-raised border border-line-200 rounded-xl flex flex-col h-full overflow-hidden">
        <div className="p-3.5 px-4 border-b border-line-200 flex items-center justify-between shrink-0">
          <div className="font-mono text-xs font-extrabold uppercase tracking-wider text-verify flex items-center gap-1.5">
          ...
          <span className="font-mono text-[11px] font-bold bg-strong px-2 py-0.5 rounded-full text-ink-700">17</span>
        </div>
        <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-3">
          <KanbanCard priority="done" badgeText="v2.8.0 Wave 8" title="Public Developer Documentation Portal (/docs)" description="Zero-AI-slop Obsidian docs portal with bespoke SVG visualizers, live hardware telemetry probe, and multi-language code snippets." persona="Route: /docs" status="Merged (3 New Tests)" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
          <KanbanCard priority="done" badgeText="v2.7.0 Wave 7" title="Multi-Model DiT Engine Adapters" description="Polymorphic BaseVideoEngine, Wan2.1 (1.3B & 14B), HunyuanVideo, and FastMCP generation tools." persona="Commit: f0f5e74" status="Merged (24 New Tests)" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
          ...
          <KanbanCard priority="done" badgeText="v2.6.0 Wave 6" title="Cockpit Local Compute HUD & Create Hybrid Switcher" description="Live Host hardware telemetry, 1-click model weight cache downloader, and dynamic $0.00 local cost telemetry." persona="Commit: Wave 6" status="Merged" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
          <KanbanCard priority="done" badgeText="v2.5.0 Wave 5" title="SpacePilot Master Landing Page" description="SkyPilot-grade marketing homepage with live bidding terminal, 4-layer stack, and wholesale price matrix." persona="Commit: a386468" status="Merged" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
          <KanbanCard priority="done" badgeText="v2.5.0 Wave 5" title="Cockpit OpenDesign Obsidian Overhaul" description="24px radius framed panels, live 1s billing odometer, and Multi-Cloud Provider Hub credential switcher." persona="Commit: 1beff14" status="Merged" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
          <KanbanCard priority="done" badgeText="v2.5.0 Wave 5" title="Unified SVG Navigation & 3-Way Theme" description="Feather/Lucide SVG UI kit headers across all 5 pages with localStorage Dark/Light/System sync." persona="Commit: 93e399c" status="Merged" statusColor="var(--verify)" borderColor="var(--verify-soft)" />
        </div>
      </section>
    </main>
  );
