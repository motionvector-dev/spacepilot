...
type EngineColor = 'emerald' | 'cyan' | 'amber' | 'purple';

const ENGINES = [
  { id: 'wan-14b', name: 'Wan2.1 14B', vram: '18GB', time: '~45s', color: 'emerald' as EngineColor },
  { id: 'wan-1.3b', name: 'Wan2.1 1.3B', vram: '8GB', time: '~12s', color: 'cyan' as EngineColor },
  { id: 'hunyuan', name: 'HunyuanVideo', vram: '16GB', time: '~35s', color: 'amber' as EngineColor },
  { id: 'ltx', name: 'LTX-2.5', vram: '10GB', time: '~18s', color: 'purple' as EngineColor },
];

export function ModelEngineStep() {
  const [selected, setSelected] = useState('ltx');
  ...
  return (
    <div className="flex flex-col gap-4 p-5 bg-[#09090b] border border-white/[0.08] rounded-xl shadow-sm">
      <div className="flex justify-between items-center">
        ...
        <span className="text-[12px] font-mono text-white/50">Local First</span>
      </div>
        ...
        {ENGINES.map(eng => {
          ...
          return (
            <button
              ...
              className={`flex flex-col gap-2.5 p-3.5 rounded-lg border text-left transition-all ${
                isSelected 
            ...
            >
              <div className="flex justify-between items-center w-full">
                ...
                {isSelected && (
                  <div className={`w-2 h-2 rounded-full ${colorMap[eng.color]}`} />
                )}
              </div>
