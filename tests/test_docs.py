...
from src.studio_api import app
...
def test_docs_js_syntax_is_valid():
    """Verify studio/docs.js syntax with node."""
    docs_js_path = PLUTO_ROOT / "studio" / "docs.js"
    assert docs_js_path.exists()

    res = subprocess.run(["node", "-c", str(docs_js_path)], capture_output=True, text=True)
    assert res.returncode == 0, f"Syntax error in studio/docs.js: {res.stderr}"
