...
each result through `spacepilot.measurements`. The point is a real
...
class RunRecord:
    ...
    the fact, and the thing `spacepilot sweep run` prints a summary of."""
    spec_id: str
...
def _resolve_job_output(outputs_dir: Path, run_id: str, job: Job) -> Path:
    ...
    `spacepilot.core.utils.resolve_output` enforces containment for reads: the
